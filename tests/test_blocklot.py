...
def test_function_validation_error(mock_env_no_access_key):
    """ blocklot validation error """
    print("*** test_function_validation_error")

    # Construct a mock HTTP request.
    req = func.HttpRequest(
        method='GET',
        body=json.dumps(mocks.BLOCKLOT_POST),
        url='/api/blocklot')
...
def test_function_success(
    ...
    req = func.HttpRequest(
        ...
        body=json.dumps(mocks.BLOCKLOT_POST).encode('utf8'),
        url='/api/blocklot')
